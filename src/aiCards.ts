...
export function buildCardVerificationPrompt(card: BoardCard, cardFilePath: string): string {
  return [
    ...
    'End the Activity entry with exactly one terminal marker on its own line. The backticked text is parser input: copy the applicable form exactly, without adding an explanation on the same line:',
    '  - `VERIFY: PASS`',
    '  - `VERIFY: FAIL: <reason>`',
    '  - `VERIFY: HUMAN: <reason>`',
