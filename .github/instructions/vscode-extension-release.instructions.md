...
description: "Use when versioning, packaging, publishing, or documenting a MWNN Kanban release. Covers patch bumps, explicit version sets, README badge sync, VS Marketplace, Open VSX, and release workflow expectations."
---

# VS Code Extension Release

- This repo is a VS Code extension published as `darrenjmcleod.mwnn-kanban`.
- For patch bumps, prefer `npm run version:build` (once defined). It should update `package.json`, `package-lock.json`, and the VS Marketplace badge in `README.md`.
...
- Release-relevant files are `package.json`, `package-lock.json`, `README.md`, and any `scripts/bump-package-version.cjs`. Check `.github/workflows/release.yml` only when publish automation itself changes.
